...
export class EventService implements IEventService {
  ...
  async createEvent(input: CreateEventInput): Promise<Result<Event, EventDetailError>> {
    ...
    if (!title?.trim()) return Err(InvalidInput("Title is required."))
    if (!location?.trim()) return Err(InvalidInput("Location is required."))
    if (!category?.trim()) return Err(InvalidInput("Category is required."))
    if (!startDatetime || !endDatetime) return Err(InvalidInput("Start and end times are required."))
    if (new Date(endDatetime) <= new Date(startDatetime)) return Err(InvalidInput("End time must be after start time."))
    if (capacity !== undefined && capacity !== null && capacity < 1) return Err(InvalidInput("Capacity must be at least 1."))
    ...
    if (!result.ok) return Err(EventNotFound("Failed to create event."))
    return Ok(result.value)
  }
